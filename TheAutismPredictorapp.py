...
model_path = '/Users/naakoshie/Downloads/final_model (2).h5'
...
#when the predict button is selected in the main application this if statement executes
    if st.button("Predict"):
        input_data = np.array([[A9, A5, A7, A6, A1, A2, A4, A8, Age_Mons, A3,]])
        new_data_scaled = loaded_scaler.transform(input_data)

        prediction_probabilities = model.predict(new_data_scaled)
        prediction = (prediction_probabilities >= 0.5).astype(int)[0]

        # Correct confidence score calculation
        confidence_score = prediction_probabilities[0][0] * 100 if prediction[0] == 1 else (1 - prediction_probabilities[0][0]) * 100
        ...
        st.title(f"Confidence Score: {confidence_score:.2f}%")
